   ...
   def new_state(self,i,j,n,action):
       ...
       if 'roll' in action and 'extra roll' not in action:
           ...
           if num_dice == 0:
               ...
               #         result.append(('',self.goal,j,n+1,prob[index]))
               #     else:
               #         result.append((next,i+add_points[index],j,n+1,prob[index]))
...
def value_iteration(hog,Value1):
               ...
               for next3,new_score,new_opponent,new_turn,new_prob in hog.new_state(score,opponent_score,turn,action):
                   if next3=='':
                       if new_score==hog.getgoal():
                           ...
                           total += (1 - Value1[(new_opponent, new_score, new_turn)]) * new_prob
                   else:
        ...
        for next1,new_score,new_opponent,new_turn,prob in hog.new_state(score,opponent_score,turn,action):
            if next1=='':
                if new_score==hog.getgoal():
                    ...
                    total+=(1-Value1[(new_opponent,new_score,new_turn)])*prob
   ...
   while True:
       ...
       for i in hog.getstate():
           for j in hog.getstate():
               for k in range(8):
                   if abs(Value1[(i,j,k)] - new_V[(i,j,k)])>max_diff:
                       max_diff=abs(Value1[(i,j,k)] - new_V[(i,j,k)])
                       print(i,j,k)
       print(max_diff)
       if max_diff<1e-6:
           break
       Value1=new_V
   return Value1, ultimate_action
   # pi = {}
   # for i in hog.getstate():
   #     for j in hog.getstate():
   #         for k in range(8):
   ...
   #                 elif (i,j,k)
   #             else:
   #                 pi[(i,j,k)] = max((Sum_value(i,j,k, action), action) for action in hog.action())[1]
   # print(ultimate_action)
   # print(Value)
   # list_of_dict = []
   # list_of_dict2 = []
   # k = 1
...
def main():
    ...
    with open('hog3.csv','r') as r:
       ...
       for row in csv_reader:
           for k in range(len(fieldname)):
               Value1[(99-count,int(fieldname[k][0]),int(fieldname[k][1]))]=float(row[str(fieldname[k])])
           count+=1
    hog=Hogmdp(100)
    Value2,policy=value_iteration(hog,Value1)
    list_of_dict = []
    list_of_dict2 = []
    k = 1
    dict1 = {}
    ...
    for key in Value2.keys():
        if k % 800 == 0:
            dict1[(key[1], key[2])] = Value2[key]
            dict2[(key[1], key[2])] = policy[key]
            list_of_dict.append(dict1)
            list_of_dict2.append(dict2)
            dict1 = {}
            dict2 = {}
            ...
            dict2[(key[1], key[2])] = policy[key]
        k = k + 1
    with open('hog3.csv', 'w', newline='') as f:
        fieldname = []
        for i in range(99, -1, -1):
            for j in range(8):
                fieldname.append((i, j))
        ...
        for dict in list_of_dict:
            writer.writerow(dict)
    with open('hog4.csv', 'w', newline='') as s:
        fieldname1 = []
        for a in range(99, -1, -1):
            for b in range(8):
                fieldname1.append((a, b))
